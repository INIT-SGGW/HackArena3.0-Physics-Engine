...
void test() {
  ...
  for (int i = 0; i < 1; i++) {
    ...
    auto [input, trans, kins] =
        world.car_manager.getCarComponents<boink::CarInput, boink::Transform,
                                           boink::Kinematics>(id);

    auto model = world.car_manager.getCarStaticComponents<boink::CarModel>();
