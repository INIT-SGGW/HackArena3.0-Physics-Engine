...
int boink_get_c_api_version(unsigned int* out_major, unsigned int* out_minor, unsigned int* out_patch)
{
  if (out_major == nullptr || out_minor == nullptr || out_patch == nullptr) return BOINK_ERR_INVALID_ARG;
...
int boink_get_engine_version(unsigned int* out_major, unsigned int* out_minor, unsigned int* out_patch)
{
  if (out_major == nullptr || out_minor == nullptr || out_patch == nullptr) return BOINK_ERR_INVALID_ARG;
...
static Eigen::Vector3d b_v3_2_e_v(const BoinkVec3& b_vec) { return Eigen::Vector3d{b_vec.x, b_vec.y, b_vec.z}; }

BoinkHandle boink_create_world(const BoinkCarModel* car_model)
{
  if (car_model == nullptr) return nullptr;
...
int boink_set_controls(BoinkHandle handle, uint64_t car_id, const struct BoinkControls* controls)
{
  boink::World* p_world = (boink::World*)handle;
  if (p_world == nullptr) return BOINK_ERR_INVALID_ARG;
...
int boink_read_car_state(BoinkHandle handle, uint64_t car_id, struct BoinkCarState* out_state)
{
  boink::World* p_world = (boink::World*)handle;
  if (p_world == nullptr) return BOINK_ERR_INVALID_ARG;
  ...
  auto components_opt =
      p_world->car_manager.getCarComponents<boink::CarInput, boink::Transform, boink::Kinematics>(car_id);
