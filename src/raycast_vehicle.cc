...
btRigidBody& btActionInterface::getFixedBody()
{
  ...
  s_fixed.setMassProps(
      btScalar(0.), btVector3(btScalar(0.), btScalar(0.), btScalar(0.)));
  return s_fixed;
}
...
namespace boink
{
      ...
      btCollisionWorld* collisionWorld, btScalar step)
  {
    ...
    for (int i = 0; i < m_wheelInfo.size(); i++)
    {
      ...
      if (suspensionForce > wheel.m_suspensionInfo.m_maxForce)
        suspensionForce = wheel.m_suspensionInfo.m_maxForce;
  ...
  void RaycastVehicle::updateWheelTransform(
      int wheelIndex, bool interpolatedTransform)
  {
  ...
  void RaycastVehicle::updateWheelTransformsWS(
      WheelInfo& wheel, bool interpolatedTransform)
  {
    ...
    wheel.m_raycastInfo.m_hardPointWS = 
      chassisTrans(wheel.m_suspensionInfo.m_chassisConnectionPointCS);
    wheel.m_raycastInfo.m_wheelDirectionWS = 
  ...
  WheelInfo& RaycastVehicle::addWheel(
  ...
  btScalar RaycastVehicle::rayCast(WheelInfo& wheel)
  {
    ...
    if (object)
    {
      ...
      btScalar minSuspensionLength = 
        ...
        wheel.m_suspensionInfo.m_maxTravelCm * btScalar(0.01);
    ...
    for (int w_it = 0; w_it < getNumWheels(); w_it++)
    {
      ...
      if (wheel_info.m_raycastInfo.m_isInContact)
      {
        ...
        // Spring
        {
          ...
          force = 
            wheel_info.m_suspensionInfo.m_stiffness * 
            length_diff * 
        ...
        // Damper
        {
          ...
          if (projected_rel_vel < btScalar(0.0))
            susp_damping = 
              wheel_info.m_suspensionInfo.m_wheelsDampingCompression;
          else
            susp_damping = 
              wheel_info.m_suspensionInfo.m_wheelsDampingRelaxation;
  ...
  struct WheelContactPoint
  {
    ...
    {
      btScalar denom0 = 
        body0->computeImpulseDenominator(
            frictionPosWorld, frictionDirectionWorld);
      btScalar denom1 = 
        body1->computeImpulseDenominator(
            frictionPosWorld, frictionDirectionWorld);
  ...
  btScalar calcRollingFriction(
      WheelContactPoint& contactPoint, int numWheelsOnGround);
  btScalar calcRollingFriction(
      WheelContactPoint& contactPoint, int numWheelsOnGround)
  {
    btScalar j1 = 0.f;
      ...
      for (int wheel = 0; wheel < getNumWheels(); wheel++)
      {
        ...
        if (groundObject)
        {
          ...
          else
          {
            ...
            rollingFriction = 
              calcRollingFriction(contactPt, numWheelsOnGround);
          }
        }
...
// fix. It only worked if car's up was along Y - VT.
  ...
  void RaycastVehicle::setCoordinateSystem(
