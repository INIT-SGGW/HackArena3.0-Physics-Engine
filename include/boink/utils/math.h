 ...
 */
Eigen::Matrix3d getRotationMatrix(double theta_x, double theta_y,
                                  double theta_z);
...
/**
 * @brief Computes a rotation matrix that rotates vector a to align with vector
 * b.
 *
 * Both input vectors must be normalized. The resulting rotation matrix R
 * satisfies: R * a_norm ≈ b_norm
 *
 ...
 * @return Rotation matrix.
 */
Eigen::Matrix3d getRotationMatrix(const Eigen::Vector3d& a_norm,
                                  const Eigen::Vector3d& b_norm);

/**
 * @brief Computes a rotation matrix using Rodrigues' rotation formula.
 *
 * Given the sine and cosine of the rotation angle and the normalized rotation
 * axis, this function returns the corresponding 3×3 rotation matrix.
 *
 ...
 */
Eigen::Matrix3d getRodriguesRotationMatrix(double sin_theta, double cos_theta,
 ...
 */
inline double deg2rad(double deg) { return deg / 180.0 * std::numbers::pi; }
}  // namespace math
}  // namespace boink
