...
struct contains_type<T, Head, Tail...> : contains_type<T, Tail...> {};

template <typename T, typename... Tail>
struct contains_type<T, T, Tail...> : std::true_type {};
...
struct type_difference<std::tuple<Ts_...>, std::tuple<Us_...>> {
 private:
  template <typename U>
  using R = std::conditional_t<contains_type<U, Ts_...>::value, std::tuple<>,
                               std::tuple<U>>;
