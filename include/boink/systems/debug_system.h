...
class DebugSystem {
 public:
  ...
  void Setup(ComponentManager<TupleStaticComponents_, TupleComponents_>&
                 component_manager,
             double dt) {
    size_t index = 0;
    auto view =
        component_manager.template getComponentView<Transform, Kinematics>();
    auto static_comps =
        component_manager.template getStaticComponentView<CarModel>();
    const auto& model = std::get<CarModel&>(static_comps);
    view.forEach([&](Transform& trans, Kinematics& kin) {
  ...
  template <typename TupleStaticComponents_, typename TupleComponents_>
  void Update(ComponentManager<TupleStaticComponents_, TupleComponents_>&
                  component_manager,
              double dt) {
    size_t index = 0;
    auto view =
 ...
 private:
  ...
  void Log(double dt, const CarModel& model, Transform& trans, Kinematics& kin,
           size_t id) {
