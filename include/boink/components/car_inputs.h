...
namespace boink
{
  struct CarInput
  {
#ifdef MEMORY_TEST
    CarInput()
      :throttle{},brake{},steer_angle{}
    {
      std::cout<<"CarInput Default contructor"<<std::endl;
    }
    CarInput(const CarInput& other)
      :throttle(other.throttle),brake(other.brake),
      steer_angle(other.steer_angle)
    {
      std::cout<<"CarInput Copy contructor"<<std::endl;
    }
    CarInput(CarInput&& other) noexcept
      :throttle(std::move(other.throttle)),brake(std::move(other.brake)),
    ...
    ~CarInput()
    {
      std::cout<<"CarInput Destructor"<<std::endl;
    }
#endif
    double throttle{};
    double brake{};
