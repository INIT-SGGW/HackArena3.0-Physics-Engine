...
class ComponentManager<std::tuple<StaticComponents_...>,
 ...
 public:
   ...
   *
   * @return A @ref ComponentView containing spans to the selected component
   * arrays.
   *
  ...
  ComponentView<SubComponents_...> getComponentView() {
    ...
    return ComponentView<SubComponents_...>{std::tuple{
        std::span(getComponentContainer<SubComponents_>().data(),
   ...
   *
   * @tparam StatcSubComponents_ The static component types to include in the
   * tuple.
   *
   ...
   *
   * @param entity_id The unique identifier of the entity to which components
   * are added.
  ...
  void addComponents(Entity::ID entity_id, Ts_&&... components) {
    ...
    if (id_to_index_map_.find(entity_id) != id_to_index_map_.end()) return;

    (getComponentContainer<std::remove_cvref_t<Ts_>>().add(
         std::forward<Ts_>(components)),
     ...);
  ...
  void updateComponents(Entity::ID entity_id, Ts_&&... sub_components) {
    ...
    if (it == id_to_index_map_.end()) return;

    (getComponentContainer<std::remove_cvref_t<Ts_>>().update(
         std::forward<Ts_>(sub_components), it->second),
     ...);
  }
   ...
   *
   * This function removes the entity's components from the storage. It
   * implements a **swap-and-pop** technique to maintain tight packing of
   * component arrays: the component data at the position of the deleted element
   * is replaced by the component data from the last active element, and then
   * the size is reduced. This invalidates the indices of the moved component,
   * which is updated in the internal mapping.
   *
  ...
  bool removeComponents(Entity::ID entity_id) {
    ...
    if (it_del == id_to_index_map_.end()) return false;
 ...
 public:
  static constexpr size_t STATIC_COMPONENTS_COUNT =
      sizeof...(StaticComponents_);
